...
| Provider | a backend that declares its provider id in its manifest and registers it | `plugins/totp-rfc6238`, `plugins/sms-twilio` |
...
(`plugins/*/workbench.plugin.json`), so a new provider plugin needs no config
